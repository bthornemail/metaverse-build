...
# Generate device plan and start plan server
bash "$ROOT/pipelines/discovery/device-plan.sh" >/dev/null
ONCE=1 PORT=7070 PLAN_FILE="$PLAN_FILE" bash "$ROOT/pipelines/discovery/plan-server.sh" >/dev/null

{
  echo "# Phase 22C — ESP32 Plan-Driven TCP Bus Transcript"
  ...
  : > /tmp/phase22C-host-sub.out
  TRACE_OUT="$ROOT/pipelines/posix-bus/tcp/state/trace.out"
  tail -n +1 -F "$TRACE_OUT" > /tmp/phase22C-host-sub.out 2>&1 &
  SUB_PID=$!
  sleep 0.2

  if [ -n "$TTY" ]; then
    echo "## Start serial monitor ($TTY)"
    TTY="$TTY" bash "$ROOT/pipelines/esp32/serial-monitor.sh" > /tmp/phase22C-serial.out 2>&1 &
    SER_PID=$!
  fi
  ...
  sleep 0.2
  PASS_COUNT=$(wc -c < /tmp/phase22C-host-sub.out | tr -d ' ')
  ...
  set -e
  sleep 0.2
  FAIL_COUNT=$(wc -c < /tmp/phase22C-host-sub.out | tr -d ' ')
  ...
  echo "Last message:"
  tail -n 1 /tmp/phase22C-host-sub.out

  if [ -n "$TTY" ]; then
    echo
    echo "### Serial monitor output"
    if [ -f /tmp/phase22C-serial.out ]; then
      sed -n '1,5p' /tmp/phase22C-serial.out
    else
